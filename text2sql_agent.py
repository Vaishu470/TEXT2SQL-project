...
def normalize_openai_error_message(error: Exception) -> str:
    message = str(error)
    if "ratelimiterror" in message.lower() or "insufficient_quota" in message.lower() or "quota" in message.lower():
        return "OpenAI quota exceeded or rate limited. Please check your OpenAI plan and billing details."
...
If the question is out of scope, set the final answer immediately and do not continue.
"""
    ...
    try:
        ...
        result = {"is_in_scope": True, "is_greeting": False}
    ...
    try:
        ...
        for i, statement in enumerate(sql_statements):
            ...
            if results:
                ...
                formatted_results = [dict(zip(column_names, row)) for row in results[:100]]
...
def viz_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if isinstance(results, dict):
            ...
            if first_key and isinstance(results[first_key], list):
...
Generate the Plotly code:"""
        ...
        exec_globals = {"df": df, "pd": pd, "json": json, "go": go, "px": px}
...
def should_retry(state: AgentState) -> str:
    if state.get("error"):
        iteration = state.get("iteration", 0)
        return "retry" if iteration <= 3 else "end"
    return "success"
...
async def process_question_stream(question: str):
    ...
    try:
        ...
        error_message = normalize_openai_error_message(e)
        yield {"type": "error", "error": error_message}


if __name__ == "__main__":
    print("text2sql_agent module loaded")
